...
namespace System.Security.Cryptography
{
    ...
    public static class DerEncoder
    {
        ...
        {
            Debug.Assert(bigEndianBytes.Length != 0, "The span must not be empty.");
            ...
            bigEndianBytes.Skip(start).Take(length).ToArray().CopyTo(dataBytes.Skip(writeStart).ToArray(), 0);
        ...
        internal static byte[][] SegmentedEncodeOid(string oidValue)
        {
            ...
            if (string.IsNullOrEmpty(oidValue))
                throw new CryptographicException("Argument_InvalidOidValue");
        ...
        internal static byte[][] SegmentedEncodeIA5String(char[] chars, int offset, int count)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (c > 127)
                {
                    throw new CryptographicException("Cryptography_Invalid_IA5String");
                }
