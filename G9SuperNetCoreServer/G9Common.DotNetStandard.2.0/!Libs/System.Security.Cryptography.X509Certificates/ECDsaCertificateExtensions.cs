    ...
    public static class ECDsaCertificateExtensions
    {
        ...
        public static X509Certificate2 CopyWithPrivateKey(this X509Certificate2 certificate, ECDsa privateKey)
        {
            ...
            if (certificate.HasPrivateKey)
                throw new InvalidOperationException("Cryptography_Cert_AlreadyHasPrivateKey");

            using (ECDsa publicKey = GetECDsaPublicKey(certificate))
            {
                if (publicKey == null)
                    throw new ArgumentException("Cryptography_PrivateKey_WrongAlgorithm");

                if (!IsSameKey(publicKey, privateKey))
                {
                    throw new ArgumentException("Cryptography_PrivateKey_DoesNotMatch", nameof(privateKey));
                }
            }
