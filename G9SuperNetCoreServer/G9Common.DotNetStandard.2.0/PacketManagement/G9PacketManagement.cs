    ...
    public class G9PacketManagement
    {
            ...
            G9PacketDataType dataType, Guid? customRequestId)
        {
            try
            {
                if (command.Length == CalculateCommandSize)
                {
                    ...
                    var requestIdByteArray = requestId.ToByteArray();
                    ...
                    if (body.Length <= CalculateBodySize)
                    {
                        ...
                        memoryStream.Write(requestIdByteArray, 0, requestIdByteArray.Length);
                    ...
                    else
                    {
                        ...
                        for (byte i = 1; i < counter; i++)
                        {
                            ...
                            using (var memoryStream = new MemoryStream())
                            {
                                ...
                                var splitPacketData = body.Skip(offset).Take(length).ToArray();
                                memoryStream.Write(splitPacketData, 0, splitPacketData.Length);
                                memoryStream.Write(requestIdByteArray, 0, requestIdByteArray.Length);
        ...
        public G9SendAndReceivePacket UnpackingRequestByData(byte[] packetData)
        {
            ...
            return new G9SendAndReceivePacket(
                ...
                // Set command
                EncodingHandler.EncodingType.GetString(packetData.Skip(AG9BaseConfig.PacketTypeSizeAndPacketDataTypeSizeAndBodySizeSpaceBusy).Take(CalculateCommandSize).ToArray()),
                // Set body
                packetData.Skip(PacketHeaderSize).Take(packetBodySize).ToArray(),
                // Set request id
                new Guid(packetData.Skip(PacketHeaderSize + packetBodySize).Take(AG9BaseConfig.PacketRequestIdSize).ToArray()));
        }
