                ...
                ((Action<string, CommandDataType<TAccount>>) accessToCommandDataType)?.Invoke(
                    CommandName, new CommandDataType<TAccount>(
                        ...
                        (data, account, requestId) =>
                        {
                            try
                            {
                                ReceiveCommand(data.FromJson<TReceiveType>(), account, requestId,
                                    (dataForSend, sendType) =>
                                    {
                                        if (sendType == CommandSendType.Asynchronous)
                                            account.SessionSendCommand.SendCommandByNameAsync(CommandName, dataForSend,
                                                requestId);
                                        else
                                            account.SessionSendCommand.SendCommandByName(CommandName, dataForSend, requestId);
                                    });
                            }
                            catch (Exception ex)
                            {
